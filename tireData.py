...
def get_clusters(df: pd.DataFrame, column: str, n_clusters: int):
    ...
    return sorted(list(centers.values()), reverse=True), [centers[label] for label in labels]
...
if __name__ == '__main__':
    ...
    path = 'B1654raw33.dat'
    df = pd.read_table(path, sep='\t', skiprows=1, header=0)
    ...
    for fz_center in fz_centers:
        sns.scatterplot(df.loc[(83, 0, fz_center)], x='SA', y='FY', s=10, ax=axes[0], label=f'{fz_center}N')
        sns.scatterplot(df.loc[(83, 0, fz_center)], x='SA', y='MZ', s=10, ax=axes[1], label=f'{fz_center}N')
    axes[0].legend()
    print("DONE")
    plt.show()
